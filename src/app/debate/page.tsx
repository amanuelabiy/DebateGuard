...
function DebateContent() {
  ...
  const handleCustomTimeChange = (value: string) => {
    ...
    if (value && !isNaN(Number(value))) {
      const seconds = timeUnit === 'minutes' ? Number(value) * 60 : Number(value);
      setInitialTime(seconds);
      setTimeLeft(seconds);
    }
  };
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-[#E5E7EB] mb-4 md:mb-0">
                Current Speaker: {currentSpeaker === 1 ? "Speaker 1" : "Speaker 2"}
              </h2>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (customTime && !isNaN(Number(customTime))) {
                            const seconds = e.target.value === 'minutes' 
                ...
                <div className="mb-4">
                  ...
                  <div className="bg-[#1F2937] p-4 rounded-md max-h-[300px] overflow-y-auto border border-[#2C3E50]">
                    {getAllTranscripts().map((segment, index) => (
                      <div key={index} className="border-b pb-2 border-[#2C3E50]">
                        <p className={`font-medium ${segment.speakerId === '1' ? 'text-[#2563EB]' : 'text-[#10B981]'}`}>
                    ...
                    {fallacies.length > 0 ? (
                      <ul className="space-y-2">
                        {fallacies.map((item, index) => (
                          <li key={index} className="border-b pb-2 border-[#2C3E50]">
                            <p className={`font-medium ${item.speakerId === '1' ? 'text-[#2563EB]' : 'text-[#10B981]'}`}>
                              {item.speakerId === '1' ? 'Speaker 1' : 'Speaker 2'}
                            </p>
                            <p className="text-red-400 font-semibold">Fallacy: {item.type}</p>
                            <p className="text-[#10B981]">Fix: {item.fix}</p>
                          </li>
                        ))}
                      </ul>
                    ) : (
                      <p className="text-[#9CA3AF]">No fallacies detected yet</p>
                    )}
                  </div>
                </div>
              </div>
