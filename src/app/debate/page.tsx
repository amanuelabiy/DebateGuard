        ...
        {debateSummary ? (
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                <h3 className="text-lg font-semibold text-blue-700 mb-2">Speaker 1</h3>
                <div className="max-h-[300px] overflow-y-auto">
                  {speaker1Transcript.map((segment, index) => (
                    <div key={index} className="mb-2 pb-2 border-b border-blue-200">
                      <p className="text-sm text-gray-500">{new Date(segment.timestamp).toLocaleTimeString()}</p>
              ...
              <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                <h3 className="text-lg font-semibold text-green-700 mb-2">Speaker 2</h3>
                <div className="max-h-[300px] overflow-y-auto">
                  {speaker2Transcript.map((segment, index) => (
                    <div key={index} className="mb-2 pb-2 border-b border-green-200">
                      <p className="text-sm text-gray-500">{new Date(segment.timestamp).toLocaleTimeString()}</p>
                      <p className="text-gray-800">{segment.text}</p>
                    </div>
                  ))}
            ...
            <div className="bg-red-50 p-4 rounded-lg border border-red-200 mb-6">
              <h3 className="text-lg font-semibold text-red-700 mb-2">Detected Fallacies</h3>
              <div className="max-h-[300px] overflow-y-auto">
                {fallacies.length > 0 ? (
                  fallacies.map((fallacy, index) => (
                    <div key={index} className="mb-4 pb-4 border-b border-red-200">
                      <div className="flex justify-between">
                        <span className="font-medium text-red-700">Speaker {fallacy.speakerId}</span>
                        <span className="text-sm text-gray-500">{new Date(fallacy.timestamp).toLocaleTimeString()}</span>
                      </div>
                      <p className="text-gray-800 mt-1"><strong>{fallacy.type}:</strong> {fallacy.description}</p>
                      <p className="text-green-700 mt-1"><strong>Fix:</strong> {fallacy.fix}</p>
                    </div>
                  ))
                ) : (
                  <p className="text-gray-600">No fallacies detected in this debate.</p>
                )}
              </div>
            </div>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-blue-700">Debate Session</h2>
              ...
              <div>
                <div className="mb-4">
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="text-lg font-semibold text-blue-700">Controls</h3>
                  </div>
                ...
                <div className="mb-4">
                  <h3 className="text-lg font-semibold text-blue-700 mb-2">Live Transcription</h3>
              ...
              <div>
                <div className="mb-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-md max-h-[300px] overflow-y-auto border border-gray-200">
                      ...
                      <div key={index} className="border-b pb-2 mb-2">
                        <div className="flex justify-between">
                          <p className="font-medium text-blue-700">
                            {segment.speakerId === '1' ? 'Speaker 1' : 'Speaker 2'}
                          </p>
                    ...
                    {getAllTranscripts().length === 0 && (
                      <p className="text-gray-500 italic">No transcript yet. Start recording to see transcriptions here.</p>
                    )}
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="bg-red-50 p-4 rounded-md max-h-[200px] overflow-y-auto border border-red-200">
                    {fallacies.length > 0 ? (
                      fallacies.map((fallacy, index) => (
                        <div key={index} className="border-b border-red-200 pb-2 mb-2">
                          <div className="flex justify-between">
                            <span className="font-medium text-red-700">Speaker {fallacy.speakerId}</span>
                          ...
                          <p className="text-gray-800"><strong>{fallacy.type}:</strong> {fallacy.description}</p>
                          <p className="text-green-700 text-sm"><strong>Fix:</strong> {fallacy.fix}</p>
                        </div>
                      ))
                    ) : (
                      <p className="text-gray-600 italic">No fallacies detected yet.</p>
                    )}
                  </div>
                </div>
              </div>
